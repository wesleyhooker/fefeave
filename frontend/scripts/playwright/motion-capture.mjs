...
async function bootstrapAuth(page, nextPath) {
  ...
  const res = await page.goto(url.toString(), { waitUntil: "load", timeout: 90000 });
  if (!res || !res.ok()) {
...
try {
  ...
  await page.screenshot({ path: out("motion-after-01-rest-shows.png"), fullPage: true });
  ...
  await page.screenshot({ path: out("motion-after-02-hover-sidebar.png"), fullPage: true });

  const logShow = page.getByRole("button", { name: /log show/i }).first();
  await logShow.waitFor({ state: "visible", timeout: 8000 });
  await logShow.hover();
  await page.waitForTimeout(120);
  await page.screenshot({ path: out("motion-after-03-hover-log-show.png"), fullPage: true });

  await logShow.click();
  await page.waitForTimeout(260);
  await page.screenshot({ path: out("motion-after-04-open-side-panel.png"), fullPage: true });

  await page.screenshot({ path: out("motion-after-05-post-panel-shows.png"), fullPage: true });

  const firstPastWeek = page.locator("section details > summary").first();
  if (await firstPastWeek.count()) {
    await firstPastWeek.hover();
    await page.waitForTimeout(120);
    await page.screenshot({ path: out("motion-after-06-hover-week-row.png"), fullPage: true });
    await firstPastWeek.click();
    await page.waitForTimeout(220);
    await page.screenshot({ path: out("motion-after-07-open-week-row.png"), fullPage: true });
  }
